...
import CSRmining.subset as ss

def CSR_apriori(S, minsup, minconf):
    print ("Running CSR-apriori on data with minsup "+str(100*minsup)+"% and minconf "+str(100*minconf)+"%...")
    ...
    for sec in S:
        ...
        y = sec[1]
        for item in condset:
            condsupCount[str(item)] += 1
            rule = concat(str(item), ',', y)
            lst.add((str(item),y))
            rulesupCount[rule] += 1
